...
class Reward(object):
    def __init__(self, lstm, update_rate):
        ...
        _, _, _, _, self.gen_x = control_flow_ops.while_loop(
            cond=lambda i, _1, _2, _3, _4: i < self.sequence_length+1,
    ...
    def get_reward(self, sess, input, rollout_num, discriminator):
        ...
        for i in range(rollout_num):
            for given_num in range(1, len(input[0])):
                ...
                y = np.squeeze(np.eye(self.num_classes)[samples[:,-1]])
                feed = {discriminator.input_x: samples[:,:-1], discriminator.label: y}
                ypred_for_auc, label_ypred_for_auc = sess.run([discriminator.ypred_for_auc, discriminator.label_ypred_for_auc], feed)
                ypred = np.array([item[1] for item in ypred_for_auc])
                label_ypred = np.array([item[1] for item in label_ypred_for_auc])
                ypred_combined = np.add(ypred, label_ypred)
                if i == 0:
                    rewards.append(ypred_combined)
                else:
                    rewards[given_num - 1] += ypred_combined

            # the last token reward
            y = np.squeeze(np.eye(self.num_classes)[input[:,-1]])
            feed = {discriminator.input_x: input[:,:-1], discriminator.label: y}
            ypred_for_auc, label_ypred_for_auc = sess.run([discriminator.ypred_for_auc, discriminator.label_ypred_for_auc], feed)
            ypred = np.array([item[1] for item in ypred_for_auc])
            label_ypred = np.array([item[1] for item in label_ypred_for_auc])
            ypred_combined = np.add(ypred, label_ypred)
            if i == 0:
                ...
                rewards[(len(input_x[0])-1)] += ypred_combined
