...
import nltk
nltk.download('punkt')  #KratikaA: added this for handling punctuation like ',' added in each line before label
...
class DocEmbSim_Conditional(Metrics):
    def __init__(self, oracle_file=None, generator_file=None, num_vocabulary = None, label_vocab = None):
        super().__init__()
        ...
        self.label_vocab = label_vocab  #KratikaA: added vocab_dict based on label
        self.valid_examples_dict = None
    ...
    def get_frequent_word_with_labels(self):
        if self.valid_examples is not None:
            return self.valid_examples
        ...
        with open(self.oracle_file, 'r') as file:
            for line in file:
                text = nltk.word_tokenize(line)
                label = int(text[-1])    #KratikaA: get label from the last index
                text = text[:-2]    #KratikaA: update text by removing label and preceding comma
                text = list(map(int, text))
                if label not in label_words:
                    label_words[label] = []
                label_words[label] += text
                # words += text

        for labels, words in label_words.items():
            counts = collections.Counter(words)
            new_list = sorted(words, key=lambda x: -counts[x])
            word_set = list(set(new_list))
            if len(word_set) < self.num_vocabulary//10:
                self.valid_examples_dict[labels] = word_set
            else:
                self.valid_examples_dict[labels] = word_set[0:self.num_vocabulary//10]

        return valid_examples_dict
    ...
    def get_wordvec(self, file):
        ...
        if self.valid_examples_dict is None:
            self.get_frequent_word_with_labels()
